	...
	let grid = {
		...
		adjust: function() {
			...
			for (let i = 0; i < container.length; i++) {
				// container
				let active_container = $(container[i]);

				// cols
				let cols = parseInt(active_container.data('cols'), 10);
				...
				// margin
				let margin = parseInt(active_container.data('margin'), 10);
				if (!margin) margin = 0;
				...
				// height
				let height = parseFloat(active_container.data('height'));
				if (!height) height = 'auto';

				// double height
				let double_height = parseFloat(active_container.data('double-height'));
				if (!double_height) double_height = 2;

				// container width
				let container_width = active_container.width();

				// items
				let items = active_container.find('.entry'),
